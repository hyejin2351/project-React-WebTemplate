...
const getClient = (uri, cookies, cacheOpts, initialData) => {
  ...
  if (!gApolloClient) {
    gApolloClient = getBrowserClient(uri, cacheOpts, initialData);
  }
  return gApolloClient;
};

module.exports = ({ url, opts }) => Component => (
  class extends React.Component {
    static getInitialProps(ctx) {
      ...
      return Promise.resolve(Component.getInitialProps ? Component.getInitialProps(ctx) : {})
        .then(childProps => Promise.resolve()
          .then(() => {
            if (!process.browser) {
              const ctxurl = {
                query: ctx.query,
              ...
              const app = (
                <ApolloProvider client={client}>
                  <Component url={ctxurl} {...childProps} />
                </ApolloProvider>
              );
...
module.exports.clear = () => gApolloClient = null;
